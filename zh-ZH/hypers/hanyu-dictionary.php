...
function get_entry()
{
  global $server_root, $entry_id, $dir_zones, $dir_templates;

  $app_title      = "Dictionary";
  $app_area       = "ln";
  $app_name       = "hanyu-dictionary";
  $dict_name      = "hanyu.sqlite";
  $app_dir        = "../{$dir_zones}/{$app_area}/zh_resources";
  $dict_dir       = "../{$dir_zones}/{$app_area}/zh_resources/databases";
...
function get_select_result($dict_db, $sql_statement)
{
  ...
  while($selected_row = $select_result->fetchArray(SQLITE3_ASSOC))
  {
    ...
    {
      $staging_string .= "<a href='../../../hypers/hanzi-strokes.php?varHanziId={$field_hant_array[$counter]}'>";
    ...
    if ($field_hans !== "")
    {
      ...
      for ($counter = 0; $counter < count($field_tones_array); $counter++)
      {
        $staging_string .= "<a href='../../../hypers/hanzi-strokes.php?varHanziId={$field_hans_array[$counter]}'>";
        $staging_string .= "<span class='clHanziBig clTone{$field_tones_array[$counter]}'>";
